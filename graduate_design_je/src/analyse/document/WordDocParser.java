...
public class WordDocParser {
	...
	private void splitToSmallFiles(String docFile, String outPutPath) throws IOException {
		...
		while(subPartsIt.hasNext()) {
			...
			// jeOutPutName 保存分词结果的文件的绝对路径
			String jeOutPutName = outPutPath + Constant.FILE_SEPARATOR + Constant.SEGMENT_DIR + Constant.FILE_SEPARATOR + docName + "_" + filePointer + ".txt";
			writer = new BufferedWriter(new FileWriter(jeOutPutName));
			writer.write(words);
			writer.close();
	...
	private ArrayList<String> splitFile(String docFile) {
	    ...
	    // 打印文本段划分结果
	    for (int i = 0; i < divideResult.size(); i++) {
	    	System.out.println(">>> " + i + " : " + divideResult.get(i));
	    }
	    //
